"""
WorldCourts scraper for The Junior Associate library.

WorldCourts provides access to decisions from international courts and tribunals.
"""
...
class WorldCourtsScraper(BaseScraper):
    """
    Scraper for WorldCourts.com - International courts and tribunals database.

    WorldCourts provides access to decisions from international courts,
    tribunals, and other international judicial bodies.
    """

    @property
    def base_url(self) -> str:
        return "https://www.worldcourts.com"
    ...
    def search_cases(
        ...
        **kwargs,
        ...
        Args:
            ...
            **kwargs: Additional parameters like document_type

        Returns:
            List of CaseData objects

        Example:
            >>> scraper = WorldCourtsScraper()
        ...
        # Build search parameters for WorldCourts
        search_params = {}
        ...
        self.logger.info(f"Found {len(cases)} cases from WorldCourts")
        return cases

    def get_case_by_id(self, case_id: str) -> Optional[CaseData]:
        """
        Retrieve a specific case by its WorldCourts ID or URL.

        Args:
            case_id: WorldCourts case ID or URL
        ...
        Example:
            >>> scraper = WorldCourtsScraper()
            >>> case = scraper.get_case_by_id("inst/iachr/decisions/2023/123")
        """
        if not case_id:
    ...
    def _parse_case_detail(self, soup, url: str) -> Optional[CaseData]:
        ...
        try:
            ...
            # Look for additional court information in text
...
def get_case_by_id(case_id: str) -> Optional[CaseData]:
    """
    Get a specific case by ID from WorldCourts.

    Args:
        case_id: WorldCourts case ID

    Returns:
        CaseData object or None

    Example:
        >>> from the_junior_associate.worldcourts import get_case_by_id
        >>> case = get_case_by_id("inst/iachr/decisions/2023/123")
        >>> if case:
        ...     print(case.case_name)
    """
    with WorldCourtsScraper() as scraper:
...
def search_cases(
    ...
    Example:
        >>> from the_junior_associate.worldcourts import search_cases
        >>> cases = search_cases("human rights", court="IACHR")
    """
    with WorldCourtsScraper() as scraper:
