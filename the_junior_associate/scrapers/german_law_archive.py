"""
German Law Archive scraper for The Junior Associate library.

German Law Archive provides access to German court decisions and legal documents.
"""
...
class GermanLawArchiveScraper(BaseScraper):
    """
    Scraper for German Law Archive - German legal database.

    Provides access to German court decisions from federal and state courts
    including the Bundesverfassungsgericht, Bundesgerichtshof, and other courts.
    """

    @property
    def base_url(self) -> str:
        return "https://germanlawarchive.iuscomp.org"
    ...
    def search_cases(
        ...
        **kwargs,
        ...
        Args:
            ...
            court: Court abbreviation (e.g., 'BVerfG', 'BGH', 'BAG')
        ...
        Example:
            >>> scraper = GermanLawArchiveScraper()
        ...
        self.logger.info(f"Found {len(cases)} cases from German Law Archive")
        return cases
    ...
    def get_case_by_id(self, case_id: str) -> Optional[CaseData]:
        ...
        Args:
            case_id: German case citation or URL
        ...
        Example:
            >>> scraper = GermanLawArchiveScraper()
            >>> case = scraper.get_case_by_id("BVerfGE 149, 222")
        """
        if not case_id:
...
def get_case_by_id(case_id: str) -> Optional[CaseData]:
    """
    Get a specific case by ID from German Law Archive.

    Args:
        case_id: German case citation

    Returns:
        CaseData object or None

    Example:
        >>> from the_junior_associate.german_law_archive import get_case_by_id
        >>> case = get_case_by_id("BVerfGE 149, 222")
        >>> if case:
        ...     print(case.case_name)
    """
    with GermanLawArchiveScraper() as scraper:
...
def search_cases(
    ...
    Example:
        >>> from the_junior_associate.german_law_archive import search_cases
        >>> cases = search_cases("constitutional law", court="BVerfG")
    """
    with GermanLawArchiveScraper() as scraper:
